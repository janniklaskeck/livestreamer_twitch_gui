...
public class DccManager {
    ...
    private PircBot bot;
    private Vector<DccFileTransfer> awaitingResume = new Vector<>();
    ...
    boolean processRequest(String nick, String login, String hostname, String request) {
        ...
        if (type.equals("SEND")) {
            ...
            DccFileTransfer transfer = new DccFileTransfer(bot, this, nick, login, hostname, type, filename, address,
                    port, size);
            ...
            if (transfer != null) {
                ...
                bot.sendCTCPCommand(nick, "DCC ACCEPT file.ext " + port + " " + progress);
            }
            ...
            final DccChat chat = new DccChat(bot, nick, login, hostname, address, port);

            new Thread() {
                @Override
                public void run() {
                    bot.onIncomingChatRequest(chat);
                }
            }.start();
        } else {
            return false;
